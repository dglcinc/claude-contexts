...
- **Weekly reboot (intentional)**: `/etc/crontab` line `0 0 * * 0 root reboot now` reboots the Pi every Sunday at midnight EDT as a routine system reset. This is deliberate, not a bug. Side effects to expect every Sunday: `journalctl --list-boots` shows a new boot ID around 00:00; every `Restart=always` service comes back up automatically (signalk, influxdb, all pivac-*); nginx is `Restart=on-failure` and also starts cleanly. The sd-clone timer runs 2 hours later at 02:00 — that 2h gap is intentional spacing. pivac-redlink eats its documented ~100s cold-start cycle on the post-reboot login (see pivac/CLAUDE.md "Known Operational Behaviours"). If a service is *not* coming back after Sunday boots, check its unit's `Restart=` setting first before assuming a real failure.
