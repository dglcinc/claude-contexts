...
**Access notes**: M2 MacBook = `david@10.0.0.83` (fixed IP, wired USB-ethernet; the `.109` originally recorded here was a Wi-Fi DHCP lease — the Wi-Fi secondary address changes, `.83` doesn't. SSH key from the Mini works; Arduino repo at `~/github/Arduino`, GUI IDE only). UniFi **APs** take SSH key auth as `dglcinc` (e.g. `KitchenAP` 10.0.0.78 — holds 2.4 GHz client logs); the **USG-3P** only takes username/password. This Mac's GitHub SSH had been broken — it caused a 25-commit-stale pivac checkout and repeated Arduino-mapping mislabeling; fixed by adding the Mini's `id_rsa` to GitHub. `gh` CLI token on the Mini is still invalid (HTTP 401).
