...
> ### ▶ ACTIVE HANDOFF — push to the phone is live and tested; the MOB tap crash is understood, mitigated and fixed upstream (2026-09-06 11:15, parallel session)
>
> `signalk-push-notifications` 2.6.0 is installed and enabled on the Pi, nginx now forwards
> `/plugins/` so WilhelmSK can register, the phone is paired, and a lowered `redlink-stale`
> sentinel fired and resolved through Grafana → poller → Signal K → bridge email twice, the second
> run with the phone registered (10:56 EDT). The app's man-overboard button then crashed WilhelmSK
> on every launch: the server fills an MOB entry's position from `navigation.position`, which a
> house lacks, and the app subscripts the resulting `NSNull` (`SignalKSource.m:1028`, unchanged on
> 1.19). Recovered with `DELETE /signalk/v2/api/notifications/<id>` plus a signalk restart; the
> house position (40.89929, −74.43873) is now seeded in `~/.signalk/baseDeltas.json`, and Wilhelm
> **#156** adds the null guard. CLAUDE.md carries the plugin, the nginx block, the test recipe
> (`f2c434d`) and the position rule (`f84a39c`); both are pulled on the Pi.
>
> **This file** now holds one handoff; the older ones are in `archive/handoffs.md` (**#16**, open),
> and `save-context` step 6 archives the previous handoff before pasting the next.
>
> **Pending pulls:** claude-contexts on the Pi and M2 after #16 merges; pivac on M2.
>
> **Next:** confirm the two 10:56 pushes reached the phone; device-test Wilhelm #155, then ship
> #155 and #156 (the 1.19 beta expired, the phone runs 1.18.0 build 227); carried board builds and
> the LoopDelta gate check; review #125 #124, then #117 #94, Arduino #10. P95 5→3 (08-30) still
> awaits a hot day. Another session pushed to pivac master this morning (#154, `2ddbf4a`), so check
...
> `docs/ds18b20-bus-topology.md` are merged (#141, #143) and name every component, rail and
> wire by `(column,row)` hole, each with a grid-accurate figure generated by a committed
> script. The next work is physical: build the two boards.
>
> **The geometry was read programmatically from the Phoenix drawings** — high-res render,
> connected-component analysis of the copper. INT board: header row k carries Pi pins 2k−1/2k,
> even pins fan to pads (3,k+1)+(4,k+1), odd to (5,k+1), **pin 1 at (2,1) confirmed**; the 16
> plug positions fan to row 2, columns 6–21; keep-outs at rows 5–6, rows 22–23, column 20.
> Resistors sit in the cathode leg so their component-side bodies are the only legal crossing
> of the upper band; **IC-C mounts rotated 180°** to share a ground rail (likeliest assembly
> mistake, flagged).
>
> **The board-to-board link is a 5-position plug at both ends.** On the INT board 3V3, SDA,
> SCL, GPIO 4 and GND sit on consecutive access pads (5,2)–(5,6) — GPIO 4 falls between SCL
> and GND, so five positions land the header on the pads with zero wires, and GPIO 4 rides as
> the parked `w1-gpio` rollback line. EXT board: grid anchored to the physical board (row 1 =
> the close-margin end — the drawing may be end-for-end), probe sockets in row 2 facing the
> short-end opening, DS2482 at columns 8 & 11 rows 12–15, every external connection pluggable
> so either board lifts out alone.
>
> **Figure gotcha worth keeping:** an SVG referenced from markdown renders standalone, so
> `currentColor` is black on every page theme — dark viewers lost the board outline and
> numbering entirely. Every figure now carries a white background rect + explicit ink; check
> renders with headless Chrome on a dark wrapper page, not qlmanage.
>
> **Next:** build the boards (each doc's dry-fit step carries the open clearance questions);
> confirm the LoopDelta gate on a real call; review #126 #125 #124, then #120 #117 #94.
