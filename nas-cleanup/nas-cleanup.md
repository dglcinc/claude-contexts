...
**OneDrive silent zeroing — discovered 2026-07-23** (adjacent, not a NAS issue). While pulling a doc off OneDrive, found the cloud copy was all-zero bytes. Full Microsoft Graph scan of david@dglc.com's OneDrive (first-16-bytes Range read of every file, MSAL device-code, `Files.Read.All`) found **141 zeroed Office files** (2016–2021, of 11,801). Every one has a **single version = the zeroed content** → no OneDrive recovery; metadata (size/dates) preserved → silent bulk corruption, not edits. Cross-referenced the 141 against the DS225+ archive (`/volume1/snashome`) — **negative** (that NAS holds home/media, not the business-OneDrive corpus: Bowling league admin 78 of 141, resumes, HVAC docs). Remaining recovery avenue: **Outlook attachment search** (M365 connector), then Time Machine / git. Tooling + artifacts: `~/onedrive-zeroed-files{-report.md,.csv,-versions.csv}`, scratchpad `graph_zero_scan.py` / `graph_versions_probe.py` / `nas_match.py`. Detection lesson: local scans undercount (materialized files only, skip online-only placeholders) — scan the cloud via Graph.
