#!/usr/bin/env python3
"""Task 2 (wing_wilhelm) — mechanical extraction for F-A and F-C.

Reads the raw wilhelm transcripts (main-thread UUID *.jsonl only, excluding
agent-*.jsonl sidechains) from the two sources enumerated in 00-inventory.md and
emits a structured dump the analyst classifies by judgment.
...
SOURCES = [
    os.path.expanduser('~/.claude/projects/-Users-utilityserver-github-wilhelm'),
    os.path.expanduser('~/.mempalace/incoming/-Users-david-github-wilhelmsk'),
]
...
def main():
    files = []
    for src in SOURCES:
        tag = 'incoming' if 'incoming' in src else 'projects'
    ...
    out = []
    out.append('# wilhelm extraction dump (Task 2)')
    out.append('')
    ...
    out.insert(out.index('## DELIVERABLE 2 — F-A INTERRUPT TRIPLES') + 1,
               f"\nTotal interrupts found across wilhelm: {total_int}")
