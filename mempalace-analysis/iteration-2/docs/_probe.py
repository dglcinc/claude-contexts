#!/usr/bin/env python3
"""Task 4 probe — locate EVERY 'interrupted by user' occurrence across the
docs transcripts (both sources), classify whether it's a genuine standalone user-turn
...
def main():
    ...
    for tag, f in files:
        ...
        if not hits:
            continue
        out.append(f"=== [{uuid}] {tag} | {len(evs)} events, {len(users)} user msgs ===")
        for i in hits:
            e = evs[i]
            role = ex.role_of(e)
