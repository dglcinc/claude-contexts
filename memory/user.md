...
- **Bench Pi 4, MAC `dc:a6:32:19:12:ee` (eth0), has dead GPIO pads: BCM 13 and 25** read low under pull-up and pull-down with nothing on the header (found 2026-09-25; it was the DS18B20 calibration Pi). **BCM 16 has a dead pull-up only**: input and driver work, so a channel on it is proven by driving the pin high (`pinctrl set 16 op dh`, then `ip pu`) and watching it drop on the short (SP-E on the pivac I/O board proven this way 2026-09-26). Never use it to bench-test CHIL or SP-C/HPCOOL, or as a pivac swap-in. It takes a DHCP lease on the UCG and shows as `raspberrypi` or, on the bench card, `pibench`. The bench card (Pi OS Lite Trixie 2026-09-15, user `pi`, password `pivac-bench`, the M2's keys, SSH and I2C on) boots any Pi 3/4/5. A new Pi arriving 2026-09-26 is the swap-in for the rev A boards.
