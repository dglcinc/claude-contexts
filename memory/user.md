...
- **MacBook Pro "M2"** (hostname `david-m2`), user **`david`** (`/Users/david`). **DHCP — the IP drifts (`.83` → `.109` → `.42` as of 2026-07-03); always address it as `david@David-M2.local` (mDNS), never a hardcoded IP.** Reachable from the Pi by key auth (verified 2026-07-03); thin MemPalace client. David's primary hands-on machine — a laptop, so often asleep/off-LAN.
