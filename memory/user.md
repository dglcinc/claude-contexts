...
- **MacBook Pro "M2" at `10.0.0.83` (FIXED — canonical)** (hostname `david-m2`), user **`david`** (`/Users/david`). The fixed IP is on the **USB ethernet adapter (`en7`, wired)**. The **Wi-Fi (`en0`) gets a second, DHCP address** (`.42` as of 2026-07-03; `.109` was an older lease) — and **mDNS `David-M2.local` resolves to the Wi-Fi address, so don't treat mDNS or any non-`.83` sighting as "the IP changed."** Always SSH `david@10.0.0.83` (key auth from the Pi verified 2026-07-03). Thin MemPalace client. David's primary hands-on machine — a laptop, so sometimes asleep/off-LAN.
